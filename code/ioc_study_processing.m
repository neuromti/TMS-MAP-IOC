...
lab = cat(2,reshape(repmat(setup.IO.label.BI,7,1),1,[]),reshape(repmat(setup.IO.label.LM,7,1),1,[]),reshape(repmat(fliplr(setup.IO.label.M1),7,1),1,[]))';
...
for si=1:size(M,1)
    for fctr = 1:size(M,3)
        ...
        g.set_title(['RMT ',num2str(setup.IO.SI(si)),' %']);
        g.set_order_options('color',raw_lab(:,fctr)');
        %g.axe_property('YLim',[25 28],'YTICK',[20:35]);
        g.axe_property('XLIM',[10 40],'YLIM',[-100 100])
        figure('Position',figpos);
        g.no_legend();
        g.draw();
        print(gcf,[folder.results.figs,setup.IO.label.ORG{fctr},'_',num2str(setup.IO.SI(si)),'_TimeCourse.tif'],'-dtiff')    
    end
end
